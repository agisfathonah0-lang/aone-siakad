...
function ProgressBar({ value, total, color }: { value: number; total: number; color: string }) {
  ...
  return (
    <div className="w-full rounded-full h-1.5 overflow-hidden" style={{ background: 'var(--muted)' }}>
            ...
            {pengumuman.map((p, i) => (
              <div key={i} className="px-5 py-3.5 transition-colors cursor-pointer group"
                ...
                <div className="flex items-start gap-2.5">
                  <div className={`w-1.5 h-1.5 rounded-full mt-1.5 shrink-0`} style={{ background: p.penting ? '#EF4444' : 'var(--muted-foreground)' }} />
                  <div>
                    <p className="text-xs font-medium leading-snug" style={{ color: 'var(--foreground)' }}>{p.judul}</p>
                ...
                <div key={a.id || i} className="px-5 py-3.5 transition-colors cursor-pointer group"
                  ...
                  <div className="flex items-start gap-2.5">
                    ...
                    <div className="flex-1 min-w-0">
                      ...
                      <p className="text-[10px] mt-1 flex items-center gap-1" style={{ color: 'var(--muted-foreground)' }}><Clock size={9} />{a.created_at ? new Date(a.created_at).toLocaleDateString('id-ID', { day: 'numeric', month: 'short', hour: '2-digit', minute: '2-digit' }) : ''}</p>
                    </div>
                  </div>
                </div>
              ))}
      ...
      <section className="grid grid-cols-1 lg:grid-cols-4 gap-4">
        <div className="bg-card rounded-xl border border-border p-5" style={{ background: 'var(--card)', borderColor: 'var(--border)' }}>
          <div className="flex items-center gap-3.5 mb-4">
            <div className="w-12 h-12 rounded-full flex items-center justify-center text-white text-base font-bold shrink-0 shadow-sm" style={{ background: 'var(--primary)' }}>
              ...
              {aktivitas.map((a: any, i: number) => (
                <div key={a.id || i} className="py-3 transition-colors cursor-pointer group"
                  ...
                  <div className="flex items-start gap-2.5">
                    ...
                    <div className="flex-1 min-w-0">
                      <p className="text-xs font-medium leading-snug" style={{ color: 'var(--foreground)' }}>{a.judul || a.message || a.deskripsi}</p>
                      <p className="text-[10px] mt-1 flex items-center gap-1" style={{ color: 'var(--muted-foreground)' }}><Clock size={9} />{a.created_at ? new Date(a.created_at).toLocaleDateString('id-ID', { day: 'numeric', month: 'short' }) : ''}</p>
                    </div>
                  </div>
                </div>
              ))}
                      ...
                      return ipData.map((d: any, i: number) => {
                        ...
                        return (
                          <div key={i} className="flex-1 flex flex-col items-center gap-0.5">
                            <span className="text-[10px] font-bold opacity-0 group-hover:opacity-100 transition-opacity" style={{ color: 'var(--muted-foreground)', fontFamily: 'var(--font-mono)' }}>{d.value.toFixed(2)}</span>
                ...
                {khsData.semesters.slice(-4).reverse().map((sem: any, i: number) => (
                  <div key={i} className="flex items-center justify-between p-2.5 rounded-lg border border-transparent transition-all"
                    ...
                    <div className="flex items-center gap-2.5">
                      <div className="w-7 h-7 rounded-md flex items-center justify-center text-[10px] font-bold" style={{ background: 'var(--muted)', color: 'var(--muted-foreground)' }}>{sem.semester}</div>
                      <div>
                    ...
                    <span className={`text-xs font-bold ${sem.ip >= 3 ? 'text-emerald-500' : sem.ip >= 2.5 ? 'text-amber-500' : 'text-red-500'}`} style={{ fontFamily: 'var(--font-mono)' }}>{sem.ip}</span>
                  </div>
                ))}
              </div>
            </>
...
export default function DashboardPage() {
  ...
  if (role === 'akademik') return <AkademikDashboard />;
