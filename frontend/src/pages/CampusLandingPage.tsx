...
function CampusLandingPage({ slug }: { slug: string }) {
  ...
  return (
    <div className="min-h-screen bg-[#faf7f2] text-[#1e293b] font-sans antialiased overflow-x-hidden">
      <header className="w-full top-0 sticky z-50 bg-[#faf7f2]/80 backdrop-blur-md border-b border-[#e2e8f0]">
        <div className="max-w-6xl mx-auto px-4 md:px-8 flex items-center justify-between h-16 md:h-20">
          ...
          <nav className="hidden md:flex items-center gap-8">
            <a href="#programs" className="text-sm font-semibold transition-colors" style={{ color: c }}>Program</a>
            <a href="#tentang" className="text-sm text-[#64748b] hover:text-[#1e293b] transition-colors">Tentang</a>
            <a href="#berita" className="text-sm text-[#64748b] hover:text-[#1e293b] transition-colors">Berita</a>
            {landingPage.showPPDB && <a href="#ppdb" className="text-sm text-[#64748b] hover:text-[#1e293b] transition-colors">PPDB</a>}
          </nav>
          <div className="flex items-center gap-3">
            <a href={`/login?tenant=${slug}`} className="text-sm font-medium text-[#64748b] hover:text-[#1e293b] transition-colors">Login</a>
      ...
      <main>
        <section className="relative min-h-[85vh] flex items-center overflow-hidden bg-[#f1f5e9]">
          <div className="absolute inset-0 z-0">
            ...
            {heroImages.length > 1 && (
              <div className="absolute bottom-6 left-1/2 -translate-x-1/2 z-20 flex gap-2">
                {heroImages.map((_, i) => (
                  <button key={i} onClick={() => setHeroIdx(i)} className="h-2 rounded-full transition-all cursor-pointer" style={{ width: i === heroIdx ? 24 : 8, backgroundColor: i === heroIdx ? c : 'rgba(0,0,0,0.12)' }} />
                ))}
              </div>
            )}
          </div>
          <div className="max-w-6xl mx-auto px-4 md:px-8 w-full relative z-10 py-20 md:py-0" style={{ display: 'grid', gridTemplateColumns: '1.2fr 0.9fr', gap: 48, alignItems: 'center' }}>
            <div>
              <div className="inline-flex items-center gap-2 px-4 py-1.5 rounded-full mb-6" style={{ backgroundColor: `${c}12`, color: c }}>
                <Sparkles size={14} />
              ...
              <p className="text-base md:text-lg text-[#64748b] max-w-lg mb-8 leading-relaxed">
                {landingPage.heroSubtitle || `Bergabunglah dengan ${tenant.nama_pt} dan wujudkan masa depanmu.`}
              </p>
              ...
              {heroImages.length > 0 ? (
                <>
                  <div className="pt-10 transform hover:-translate-y-2 transition-transform duration-500">
                    <img src={heroImages[0]} alt="" className="w-full h-[400px] object-cover rounded-xl shadow-2xl" style={{ border: '4px solid rgba(255,255,255,0.5)' }} />
                  </div>
                  <div className="pb-10 transform hover:translate-y-2 transition-transform duration-500">
                    <img src={heroImages[heroImages.length > 1 ? 1 : 0]} alt="" className="w-full h-[400px] object-cover rounded-xl shadow-2xl" style={{ border: '4px solid rgba(255,255,255,0.5)' }} />
                  </div>
                </>
              ) : (
        ...
        <section className="py-16 md:py-20 max-w-6xl mx-auto px-4 md:px-8 -mt-10 relative z-20">
          <div className="grid grid-cols-2 md:grid-cols-4 gap-3 md:gap-4">
            {[
              ...
              <div key={i} className="bg-white p-5 rounded-xl shadow-sm border border-[#e2e8f0] text-center hover:-translate-y-1 transition-all hover:shadow-md">
          ...
          <section id="tentang" className="py-16 md:py-24 bg-white overflow-hidden">
            <div className="max-w-6xl mx-auto px-4 md:px-8">
              <div className="text-center mb-12">
                <h2 className="text-2xl md:text-4xl font-bold tracking-tight text-[#1e293b]">Tentang {tenant.singkatan || tenant.nama_pt}</h2>
                <p className="text-[#64748b] mt-3 max-w-md mx-auto">Komitmen kami terhadap pendidikan berkualitas</p>
              </div>
              <div className="flex gap-6 overflow-x-auto pb-4 no-scrollbar">
                {sambutan.active && (
                  <div className="min-w-[340px] md:min-w-[400px] bg-[#faf7f2] p-8 rounded-xl shadow-sm border border-[#e2e8f0] flex flex-col gap-6 hover:-translate-y-2 transition-transform duration-300">
                    ...
                    <p className="text-base leading-relaxed text-[#64748b] italic">{sambutan.content || `${landingPage.heroTitle}`}</p>
                    <div className="mt-auto pt-6 border-t border-[#e2e8f0] flex items-center gap-3">
                      ...
                      <div>
                        ...
                        <p className="text-xs text-[#64748b]">{sambutan.jabatan || 'Pimpinan'}</p>
                      </div>
                    </div>
                  </div>
                )}
                {prestasi.map((p, i) => {
                  ...
                  return (
                    <div key={i} className="min-w-[280px] bg-[#faf7f2] p-8 rounded-xl shadow-sm border border-[#e2e8f0] flex flex-col gap-4 hover:-translate-y-2 transition-transform duration-300">
        ...
        {landingPage.showProdi && programStudi.length > 0 && (
          <section id="programs" className="py-16 md:py-24 bg-[#faf7f2]">
            <div className="max-w-6xl mx-auto px-4 md:px-8">
              <div className="text-center max-w-2xl mx-auto mb-12">
                <h2 className="text-2xl md:text-4xl font-bold tracking-tight text-[#1e293b]">Program Studi</h2>
                <p className="text-[#64748b] mt-3">Pilihan program studi berkualitas untuk masa depanmu</p>
              </div>
              <div className="grid md:grid-cols-12 gap-4 md:gap-6">
                ...
                {programStudi.slice(3).map(p => (
                  <div key={p.id} className="md:col-span-3 bg-white p-5 rounded-xl shadow-sm border border-[#e2e8f0] hover:-translate-y-1 transition-all hover:shadow-md">
                    <span className="text-[11px] font-bold px-2 py-0.5 rounded inline-block mb-2" style={{ backgroundColor: `${c}10`, color: c }}>{p.jenjang}</span>
        ...
        {landingPage.showPromosi && promosi.length > 0 && (
          <section className="py-16 md:py-24 bg-white">
            <div className="max-w-6xl mx-auto px-4 md:px-8">
              <div className="text-center mb-12">
                <h2 className="text-2xl md:text-4xl font-bold tracking-tight text-[#1e293b]">Informasi & Promosi</h2>
                <p className="text-[#64748b] mt-3">Kegiatan dan program terbaru</p>
              </div>
              <div className="grid md:grid-cols-3 gap-4 md:gap-6">
                {promosi.map((p, i) => (
                  <div key={i} className="group bg-white rounded-xl overflow-hidden shadow-sm border border-[#e2e8f0] hover:-translate-y-1 transition-all hover:shadow-md">
                    {p.image && (
        ...
        {landingPage.showStruktur && strukturOrganisasi.length > 0 && (
          <section className="py-16 md:py-24 bg-[#faf7f2]">
            <div className="max-w-6xl mx-auto px-4 md:px-8">
              <div className="text-center mb-12">
                <h2 className="text-2xl md:text-4xl font-bold tracking-tight text-[#1e293b]">Struktur Organisasi</h2>
                <p className="text-[#64748b] mt-3">Tim kepemimpinan kami</p>
              </div>
              <div className="grid grid-cols-2 md:grid-cols-4 lg:grid-cols-5 gap-4">
                {strukturOrganisasi.map((s) => (
                  <div key={s.id} className="bg-white rounded-xl p-5 text-center shadow-sm border border-[#e2e8f0] hover:-translate-y-1 transition-all hover:shadow-md">
                    <div className="w-16 h-16 rounded-full mx-auto mb-3 overflow-hidden ring-2 ring-[#e2e8f0] bg-[#faf7f2] flex items-center justify-center">
                      {s.image ? <img src={s.image} alt={s.nama} className="w-full h-full object-cover" /> : <Users size={20} className="text-[#94a3b8]" />}
                    </div>
        ...
        {landingPage.showBerita && berita.length > 0 && (
          <section id="berita" className="py-16 md:py-24 bg-white">
            <div className="max-w-6xl mx-auto px-4 md:px-8">
              <div className="text-center mb-12">
                <h2 className="text-2xl md:text-4xl font-bold tracking-tight text-[#1e293b]">Berita Terkini</h2>
                <p className="text-[#64748b] mt-3">Informasi dan kegiatan terbaru</p>
              </div>
              <div className="grid md:grid-cols-3 gap-6">
                {berita.map(b => (
                  <div key={b.id} className="bg-[#faf7f2] rounded-xl overflow-hidden shadow-sm border border-[#e2e8f0] hover:-translate-y-1 transition-all hover:shadow-md">
                    {b.gambar && (
                      <div className="relative h-44 overflow-hidden">
        ...
        {landingPage.showPPDB && (
          <section id="ppdb" className="py-16 md:py-24 relative overflow-hidden" style={{ backgroundColor: '#0f172a' }}>
            <div className="absolute inset-0 opacity-[0.03]" style={{ backgroundImage: `url("data:image/svg+xml,%3Csvg width='60' height='60' xmlns='http://www.w3.org/2000/svg'%3E%3Cpath d='M0 0h60v60H0z' fill='none'/%3E%3Cpath d='M30 0v60M0 30h60' stroke='%23fff' stroke-width='0.5'/%3E%3C/svg%3E")` }} />
            <div className="max-w-6xl mx-auto px-4 md:px-8 relative z-10 text-center">
              ...
              <p className="text-[#94a3b8] text-base md:text-lg max-w-xl mx-auto mb-10">Daftar sekarang dan mulai perjalanan akademikmu bersama {tenant.nama_pt}.</p>
              <div className="flex flex-col md:flex-row justify-center items-center gap-4">
                ...
                <a href={`/login?tenant=${slug}`} className="px-10 py-4 border-2 border-white/20 text-white/80 rounded-lg text-sm font-bold hover:bg-white hover:text-[#0f172a] transition-all">
                  Portal Akademik
                </a>
              </div>
            </div>
        ...
        <div className="max-w-6xl mx-auto px-4 md:px-8 py-12">
          <div className="flex flex-col md:flex-row justify-between items-start gap-8">
            <div className="flex flex-col items-start gap-3">
              ...
              <p className="text-sm text-[#64748b] max-w-xs">{tenant.nama_pt} — Empowering Future Leaders.</p>
            </div>
            <div className="grid grid-cols-2 md:grid-cols-3 gap-8">
              <div>
                <p className="text-xs font-bold uppercase tracking-wider text-[#64748b] mb-3">Kontak</p>
                <div className="space-y-2">
                  ...
                  {tenant.alamat && <p className="text-sm text-[#64748b] flex items-start gap-2"><MapPin size={12} className="mt-1 shrink-0" /> {tenant.alamat}</p>}
                </div>
              </div>
              <div>
                <p className="text-xs font-bold uppercase tracking-wider text-[#64748b] mb-3">Akses</p>
                <div className="space-y-2">
          ...
          <div className="border-t border-[#e2e8f0] mt-8 pt-6 flex flex-col md:flex-row items-center justify-between gap-4">
          ...
          <div className="max-w-md w-full rounded-2xl bg-white border border-[#e2e8f0] overflow-hidden shadow-2xl" onClick={e => e.stopPropagation()}>
            ...
            <div className="p-5">
              ...
              <div className="flex items-center gap-2 mt-4">
                <button onClick={() => { setShowPopup(false); localStorage.setItem(`popup_${slug}_seen`, 'true'); }} className="px-3 py-1.5 text-xs text-[#64748b] hover:text-[#1e293b] transition-colors">
