...
app.use(rateLimit({
  windowMs: 15 * 60 * 1000,
  max: 2000,
  skip: (req) => !!(req.path === '/api/v1/health' || req.path.startsWith('/api/v1/public/') || req.path === '/api/v1/ppdb/register' || /\.(js|css|png|jpg|jpeg|gif|ico|svg|webp|woff2?|ttf|eot)$/.test(req.path)),
  handler: (req, res) => {
    if (req.accepts('html')) {
