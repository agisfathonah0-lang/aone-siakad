...
const CCTV_ROLES = [Role.ADMIN, Role.AKADEMIK, Role.KAPRODI, Role.REKTOR, Role.DEKAN];

router.get('/', authenticate, requireRole(...CCTV_ROLES), async (req: Request, res: Response, next: NextFunction) => {
  try {
...
router.get('/:id/snapshot', authenticate, requireRole(...CCTV_ROLES), async (req: Request, res: Response, next: NextFunction) => {
  try {
    if (!req.tenant) throw new AppError(400, 'Tenant tidak terdeteksi');
    const { rows } = await query(
...
router.post('/:id/stream/start', authenticate, requireRole(...CCTV_ROLES), async (req: Request, res: Response, next: NextFunction) => {
  try {
    if (!req.tenant) throw new AppError(400, 'Tenant tidak terdeteksi');
...
router.post('/:id/stream/stop', authenticate, requireRole(...CCTV_ROLES), async (req: Request, res: Response, next: NextFunction) => {
  try {
...
router.get('/:id/stream/status', authenticate, requireRole(...CCTV_ROLES), async (req: Request, res: Response, next: NextFunction) => {
  try {
...
router.get('/stream/:cameraId/:file', async (req: Request, res: Response, next: NextFunction) => {
  try {
