...
router.get(
  ...
  async (req: Request, res: Response, next: NextFunction) => {
    try {
      ...
      if (q) {
        conditions.push(`(d.nidn ILIKE $${params.length + 1} OR d.nama ILIKE $${params.length + 1})`);
        params.push(`%${q}%`);
      }
      if (programStudiId) {
        conditions.push(`d.program_studi_id = $${params.length + 1}`);
      ...
      const { rows: totalRows } = await query(
        `SELECT COUNT(*) as count FROM ${s}.dosen d ${where}`, params
      );
