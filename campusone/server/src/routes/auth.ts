...
router.post('/demo', (req: Request, res: Response) => {
  const { role } = req.body;
  const validRoles = ['SUPER_ADMIN', 'AKADEMIK', 'KEUANGAN', 'DOSEN', 'MAHASISWA', 'PMB_APPLICANT'];
  if (!validRoles.includes(role)) {
    res.status(400).json({ error: 'Role tidak valid' });
    return;
  }
  const db = getDb();
  const user = db.prepare('SELECT * FROM users WHERE role = ? LIMIT 1').get(role) as any;
  if (!user) {
